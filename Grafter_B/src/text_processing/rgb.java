...
public class rgb {
		...
		for (int i = 0; i < text.split("").length; i++)
		{
			if (text.split("")[i].equals(" "))
			{
				ret.append(ChatColor.of(get_color(
						...
						get255int(colors, interval, shift + interval, i, 1, 2)

						,
						get255int(colors, interval, shift + interval, i, 3, 4)

						,
						get255int(colors, interval, shift + interval, i, 5, 6)))).append("█");
				continue ;
			}
	...
	public static ComponentBuilder sendMessageGradient(ChatColor color0, ChatColor color1, String text, HoverEvent hover, ClickEvent click)
	{
		...
		BaseComponent[] components = mess.create();

		for (BaseComponent component : components)
			components[0].addExtra(component);
